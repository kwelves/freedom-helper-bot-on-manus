...
FORMAT4_TEXTS = [
    ...
    "📖 На сегодняшнем собрании читаем литературу. Ждём вас!",
    "📖 Формат собрания сегодня — чтение литературы АН. Вместе сильнее!",
]

# ID последнего напоминания (для reply)
...
async def send_meeting_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        last_reminder_message_id = sent_msg.message_id
        logger.info(f"Напоминание о собрании в {meeting_time} отправлено. msg_id={sent_msg.message_id}")
...
async def cmd_format4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отправить формат собрания (чтение литературы) как ответ на последнее напоминание."""
    if not is_authorized(update.effective_user.id):
        return

    global last_reminder_message_id
    format_text = random.choice(FORMAT4_TEXTS)

    try:
        await context.bot.send_message(
            chat_id=CHAT_ID,
            text=format_text,
            reply_to_message_id=last_reminder_message_id
        )
        await update.message.reply_text("✅ Формат собрания отправлен в группу.")
    except Exception as e:
        await update.message.reply_text(f"❌ Ошибка: {e}")
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "/format4 — отправить формат собрания (чтение литературы)\n"
...
def main():
    ...
    application.add_handler(CommandHandler("format4", cmd_format4))
